...
## The Decal Maker texture crash: the engine got stricter, the plugin was already wrong

**The failure, exactly where it happened.** `artifacts\manual-play-prefix-leak.log` is 1 647 591 770 B and 21 618 842 lines, and it is an **editor** log, not a player one: `C:\Program Files\Unity\Hub\Editor\2021.3.45f2\Editor\Unity.exe` at `:31`, `Initialize engine version: 2021.3.45f2 (88f88f591b2e)` at `:136`, `-executeMethod RebuildGate.ManualPlay` at `:37`, and `UnityPlayer.dll` never appears in it. Line **2928** is `Compressed TextureFormat RGBA Compressed DXT5|BC3 requires a texture size that is a multiple of 4`, `:2941` is `[C:\build\output\unity\unity\Runtime\Graphics\Texture2D.cpp line 563]` and `:2943` is `UnityException: Failed to create texture because of invalid parameters.`; the managed frames under it run `UnityEngine.Texture2D..ctor(int,int,TextureFormat,bool)` (`:2946`) ← `VAM_Decal_Maker.Decal_Maker.GetResource` (`:2947`) ← `VAM_Decal_Maker.RenderPanelBase.ConvertNormal` (`:2948`) ← `VAM_Decal_Maker.ManagerPanel.UpdateSkinImage` (`:2949`) ← `ManagerPanel.CoreEvent` (`:2950`) ← `Decal_Maker.OnCoreChange` ← `Decal_Maker+<CharacterUpdated>c__Iterator1.MoveNext`. This log mixes CRLF and LF endings, so the "line 2935" quoted elsewhere for this message is the CR-only count - under LF it is **2928**, and every line number in this section is LF. The rule it broke is visible in the frame directly under the message: 2021.3 routes the legacy `TextureFormat` constructor through `Texture2D:Internal_Create (Texture2D,int,int,int,Experimental.Rendering.GraphicsFormat,TextureColorSpace,TextureCreationFlags,intptr)` (`:2930`), and that is where a size which cannot be compressed is now refused. Counted over the whole 1.6 GB file by streaming it in 16 MB chunks (`artifacts\decal-census\full_scan.tsv`): `requires a texture size that is a multiple of 4` **1**, `Failed to create texture because of invalid parameters` **1**, `Texture2D.cpp line 563` **1**, `Decal_Maker` **26**, `Chokaphi` **0**, `Assertion failed on expression` **0**, `d3d11: failed` **0**.

#### Census: every log on this machine, by engine

`artifacts\decal-census\census2.py` → `census-decal.tsv` walks `VAMOpen\artifacts`, `C:\Games\VaM_Updater\logs` and `C:\Users\f1am3d\AppData\LocalLow\MeshedVR`, reads 455 `*.log`/`*.txt` files in 16 MB chunks, and skips files over 200 MB - which is why the 1.6 GB log appears in this table only through its full-file counts above. Grouped by the `Initialize engine version:` header each file carries, with the needle set of `census2.py`:

| Engine | files | `multiple of 4` | `Failed to create texture` | `Texture2D.cpp line 563` | `Decal_Maker` | `d3d11: failed` | `Assertion failed on expression` |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 2018.1.9f2 (the original `VaM.exe`) | 95 | 0 | 0 | 0 | **0** | 0 | 239 |
| 2018.4.36f1 (our first hop) | 4 | 0 | 0 | 0 | **0** | 0 | 1 |
| 2019.4.41f2 | 29 | 0 | 0 | 0 | **0** | 0 | 0 |
| 2020.3.49f1 | 26 | **0** | **0** | 0 | 216 | 12 | 9 |
| 2021.3.45f2 | 15 | **13** | **2** | 1 | 13 | 0 | 0 |
| 6000.1.11f1 (`VaM2`, not this project) | 2 | 0 | 0 | 0 | 0 | 0 | 0 |
| no engine header (284 files) | 284 | - | - | - | - | - | - |

The last row is not engine logs after all: those 284 files are harness `*.report.txt` dumps, shaders, licences and my own derivative files, and every hit in it is a copy - `decal-texture-probe.log` 12x `Failed to create texture`, `plugin-mentions.txt` 1x the message + 26x `Decal_Maker`, `player_literals.txt` 28x the needle (it is a listing of the engine binaries' strings), and the three 2020.3 `*.report.txt` files 2x `d3d11: failed`, 1x assert and 1x `mipLevel < m_MipCount` each, copied out of the logs they report on. The 239 asserts under 2018.1.9f2 and 1 under 2018.4.36f1 are other assert sites, not this one - the Decal-relevant assert `mipLevel < m_MipCount` is **0** under both, and the 2020.3 row's 9 are the Decal ones.

The rows that matter for the reading, one by one:

| File | Engine | Lines | `multiple of 4` | `Failed to create texture` | `Decal_Maker` |
| --- | --- | --- | --- | --- | --- |
| `...\LocalLow\MeshedVR\VaM\output_log.txt` (original install's own player) | 2018.1.9f2 | 152 | 0 | 0 | 0 |
| `...\LocalLow\MeshedVR\VaM\Player.log` / `Player-prev.log` | 2019.4.41f2 | 93 / 199 | 0 | 0 | 0 |
| `...\LocalLow\MeshedVR\VaM2\Player.log` / `Player-prev.log` | 6000.1.11f1 | 393 / 391 | 0 | 0 | 0 |
| `C:\Games\VaM_Updater\logs\patch.log` | no header | 715 | 0 | 0 | 0 |
| `artifacts\manual-play-layout-reverted.log` | 2018.1.9f2 | 828 235 | 0 | 0 | 0 |
| `artifacts\smoke-materials.log` / `smoke-src.log` / `smoke-env.log` | 2018.1.9f2 | 770 302 / 533 560 / 513 884 | 0 | 0 | 0 |
| `artifacts\logs\hop1-2018.4.36f1.log` | 2018.4.36f1 | 17 673 | 0 | 0 | 0 |
| `artifacts\logs\hop2-2019.4.41f2*.log` (4 files) and `hop3-baseline-play.log` | 2019.4.41f2 | 1 153…23 002 / 7 069 | 0 | 0 | 0 |
| `artifacts\logs\hop4-baseline-ladyclown-smoke.log` and its two twins | 2020.3.49f1 | 7 167 / 7 815 / 7 976 | **0** | **0** | 72 |
| `artifacts\manual-play-prefix-leak.log` | 2021.3.45f2 | 21 618 842 | **1** | **1** | 26 |
| `artifacts\decal-repro.log` | 2021.3.45f2 | 1 370 | **13** | **2** | 13 |

The LocalLow copies that sit outside the `.log`/`.txt` walk (`output_log.txt.before-thumbfix`, `output_log.txt.before-browserfix` and the `Player-prev` files of earlier hops) were scanned directly by the same needle set: 0 hits, all of them. **The reading that decides the question is the `Decal_Maker` column from 2018.1.9f2 down through 2019.4.41f2: 0 in 95 + 4 + 29 files.** No run on 2018.1, 2018.4 or 2019.4 on this machine ever reached the plugin at all - and the `Decal` strings those logs do contain are unrelated, `CausticDecal.cs` importer lines (`hop1-2018.4.36f1.log:3378`, `hop2-2019.4.41f2.log:1695`) and the shader property `_DecalTex` (`smoke-materials.log:764817`, `hop3-baseline-play.log:4491`). That is also why the Decal Maker's behaviour on the 2018 engines is *unsettled* rather than *void* further down. What the table does settle is the hop itself: the same plugin, on the same asset, on the two engines either side of our last hop, is tolerated on 2020.3.49f1 and fatal on 2021.3.45f2.
#### When the engine started refusing it

**The change is documented in 2021.2, and it was deliberately not backported to 2020.3.** The 2021.2.0 release notes carry, inside their Changes section (`<h4 id="changes">` at byte offset 287 072 of the cached page, the bullet itself at 303 775): *"Graphics: Restricted DXT/BCn texture compression to textures with multiple-of-four width and height. This ensures the same behavior as the Texture Importer and requires multiple-of-four dimensions when compressing."* - `https://unity.com/releases/editor/whats-new/2021.2.0`, cached at `artifacts\decal-census\relnotes-2021.2.0.html`. The needle `multiple-of-four` occurs **4 times on that page and 0 times** on the cached 2020.3.0, 2021.1.0 and 2021.3.0 pages, so 2021.2 is the release that introduced it - which places it inside our last hop, 2020.3.49f1 → 2021.3.45f2, and in no earlier one. The Unity Issue Tracker entry for the same change, `https://issuetracker.unity.com/issues/6704/texture2d-creates-dxt-texture-when-the-unacceptable-texture-size-is-given`, adds the two facts the question actually needs: the fix landed in 2021.2 and *"will not be backported to 2020.3 since it is a change that may potentially break existing (and fully functional) user projects"*, and what the old behaviour was, verbatim: *"Note: in Unity 2020.3, DXT-textures with non-multiple-of-four sizes are stored in compressed form, but uploaded to GPU in uncompressed form. This behind-your-back-decompression has been dropped in Unity 2021.2 in favor of simply not allowing this compression to begin with."*

**Our own logs land exactly on that boundary.** Three 2020.3.49f1 editor runs execute the same plugin frame with the same request and do not throw. `artifacts\logs\hop4-baseline-ladyclown-smoke.log` (engine header `Initialize engine version: 2020.3.49f1 (18249dd5551b)`, 852 549 B, 7 167 CRLF lines) prints `Assertion failed on expression: 'mipLevel < m_MipCount'` at `:1223` and `:1238`, `[C:\build\output\unity\unity\Runtime/Graphics/SharedTextureData.cpp line 99]` at `:1236` and `:1251`, then `d3d11: failed to create 2D texture id=2645 width=1 height=1 mips=3 dxgifmt=29 [D3D error was 80070057]` at `:1253` - `0x80070057` is `E_INVALIDARG` - the same failure for the shader resource view at `:1268`, `d3d11: Failed to create 2D texture in GfxDeviceD3D11` at `:1283`, and all three again in the run's own summary block at `:6929-6932`. Each of those frames carries the identical managed stack `Texture2D:Internal_Create (…,Experimental.Rendering.GraphicsFormat,…)` (`:1225`) ← `Texture2D:.ctor (int,int,UnityEngine.TextureFormat,int,bool,intptr)` (`:1226`) ← `Texture2D:.ctor (int,int,UnityEngine.TextureFormat,bool)` (`:1227`) ← `VAM_Decal_Maker.Decal_Maker:GetResource (string,bool)` (`:1228`) ← `RenderPanelBase:ConvertNormal` ← `ManagerPanel:UpdateSkinImage`. Then the run keeps going: 72 `Decal_Maker` lines (138 occurrences of the string), **0** `multiple of 4`, **0** `Failed to create texture`, 6 case-insensitive `d3d11: failed` lines of which 3 are the summary echo - and its twins `hop4-baseline2-ladyclown-run1.log` and `run2.log` are identical in all of those counts. The five `UnityException` lines the file does contain are unrelated - `Range is not allowed to be called from a MonoBehaviour constructor` at `:1037` and `:1068` and a `VRAdultFun.EmotionEngine` type-initializer failure at `:1099`, of which the run's own summary keeps the first at `:6915` and the second at `:6916` - so the texture refusal is not among them, while the texture failure itself **is** in that summary, as `[Assert]` at `:6929` and `[Error]` at `:6930-6932`, next to 25 other counted errors, and the run still ends normally. Line numbers here are `Select-String`/`Get-Content` numbering; the file has 23 lone-CR line breaks, so a pure-LF count reads 23 lower by end of file (3 lower at `:1223`). So under 2020.3 the C# texture object is constructed and only the GPU upload complains, a logged and tolerated failure; under 2021.3 the constructor itself throws and the coroutine dies. Same call, same asset, two engines, two outcomes, and nothing but the engine changed between them - the log's own `width=1 height=1` is the size the plugin asked for.

**The literals inside the installed engines agree, and they bound the change to one file format string.** `artifacts\decal-census\engine_literals_full.py` → `engine_literals_full.tsv` scans 30 binaries - the six `Unity.exe`-based editors installed under `C:\Program Files\Unity\Hub\Editor` (2018.1.9f2, 2018.4.36f1, 2019.4.41f2, 2020.3.49f1, 2021.3.45f2, 2022.3.62f3), each as its `Editor\Unity.exe` plus the development and non-development win32 and win64 `UnityPlayer.dll` from its `windowsstandalonesupport` variations, where present - for 11 needles, each scan carrying positive controls. The two remaining installed editors are scanned by `engine_literals_extra.py` → `engine_literals_extra.tsv`: they ship a ~170/280 KB launcher stub at `Editor\Unity.exe` and the real editor as `Editor\Unity.dll` (72 454 904 B for 2023.1.22f1, 93 363 112 B for 6000.6.2f1), which is why they are a separate file. The values below are the count in the version's **editor**, and `every binary` or `editor only` says how the four player binaries of the same version read:

| Needle | 2018.1.9f2 | 2018.4.36f1 | 2019.4.41f2 | 2020.3.49f1 | 2021.3.45f2 | 2022.3.62f3 |
| --- | --- | --- | --- | --- | --- | --- |
| `Compressed TextureFormat %s requires a texture size that is a multiple of %d` | **0** *(0 in all 5)* | **0** *(0 in all 5)* | 0 *(0 in all 5)* | 1 *(1 in all 5)* | 1 *(1 in all 5)* | 1 *(1 in all 5)* |
| `Crunched TextureFormat %s requires a texture size that is a multiple of %d` | **0** *(0 in all 5)* | **0** *(0 in all 5)* | 1 *(1 in all 5)* | 1 *(1 in all 5)* | 1 *(1 in all 5)* | 1 *(1 in all 5)* |
| `requires a texture size that is a multiple of ` (the tail the two literals above share, so its count is their sum) | 0 | 0 | 1 *(1 in all 5)* | 2 *(2 in all 5)* | 2 *(2 in all 5)* | 2 *(2 in all 5)* |
| `mipLevel < m_MipCount` | 1 *(editor only)* | 1 *(editor only)* | 1 *(editor only)* | 1 *(editor only)* | 1 *(editor only)* | 1 *(editor only)* |
| `GraphicsFormat` | **0** *(0 in all 5)* | 86 *(42-140)* | 218 *(59-206)* | 129 *(62-218)* | 134 *(65-225)* | 157 *(76-247)* |
| `Failed to create texture because of invalid parameters` | 0 | 0 | 0 | 0 | 0 | 0 |
| `Texture2D.cpp line 563` | 0 | 0 | 0 | 0 | 0 | 0 |
| `D3D11` (control) | 67 *(15-67)* | 87 *(15-87)* | 133 *(25-133)* | 79 *(31-218)* | 79 *(31-225)* | 82 *(33-247)* |
| `dxgi` (control) | 5 *(4-5)* | 5 *(4-5)* | 6 *(4-6)* | 11 *(6-11)* | 15 *(10-15)* | 15 *(10-15)* |

The last two rows are why the zeroes above them can be believed: the same scanner finds the controls in every one of the 30 binaries of `engine_literals_full.tsv`, `UnityEngine` in the thousands (6 504…9 665) and `NullReferenceException` 1-2. Two rows deserve their own sentence. `mipLevel < m_MipCount` - the assert the 2020.3 editor printed at `hop4-baseline-ladyclown-smoke.log:1223` - is present in **every editor and no player**, which is why that symptom appears in our editor runs and would not appear in a player build. And the short form of the refusal message, `requires a texture size that is a multiple of `, is **not a message of its own**: it is the tail the two literals above share, so its count is exactly their sum, 0/0/1/2/2/2 across the six editors and the same number in all five binaries of every version. That row is also the one place an earlier pass of this scan contradicted itself, and the disagreement was the scanner's rather than the engine's: the needles were counted over 8 MB chunks carrying only 60 bytes across each read boundary, and a needle longer than the carry can be split and lost, which is what happened to `Compressed TextureFormat %s…` at 2019.4 - the chunked pass read 1 there, whole-file reads give 0, and the table above holds the 0. A needle shorter than the carry, like this tail, was never at risk. `Failed to create texture because of invalid parameters` and `Texture2D.cpp line 563` are **0 in all 29 binaries** because both are assembled at runtime inside native code, so their absence carries no information; the *presence* of the multiple-of-four format string does. It is absent from every 2018.1.9f2 and every 2018.4.36f1 binary - editor and all four Windows player variations each - so a 2018.1 or 2018.4 engine **could not have refused this call with this error, because the message did not exist in it**. The `Crunched` sibling of the same message shows where the size validation did arrive: 0 under 2018.x, 1 from 2019.4 on. In other words the validation came in through the crunched path at 2019.4, reached the compressed path by 2020.3, and only became fatal in 2021.2 - the release note and the Issue Tracker entry above say the same thing normatively, and our 2020.3/2021.3 logs say it operationally. One qualifier belongs on that middle step, because a literal's presence in a binary and a message's emission in a run are two different measurements: the compressed literal is present in the 2020.3 editor as well, yet the 2020.3 logs never print it for this call (0 occurrences in all three baseline twins), which is precisely why the request was tolerated there while the device still refused the upload. The literal is therefore a bound on the **2018.x** half of the ladder only, where the string does not exist in any of the ten binaries.

**What the same scan says about the two newer editors, and where the method stops.** In 2023.1.22f1 and 6000.6.2f1 both `TextureFormat`-worded multiple-of-four format strings read **0** in the editor, while the same files carry every control (`GraphicsFormat` 158/162 in the editors, `D3D11` 79/95, `dxgi` 14/14, `UnityEngine` 9 883/11 350). The reason is a **rewording, and it is measured rather than inferred**: the tail literal `requires a texture size that is a multiple of ` still reads 1 in each of those two files, and the 190 bytes around the hit print `Compressed GraphicsFormat %s requires a texture size that is a multiple of %d` - the same message with `TextureFormat` replaced by `GraphicsFormat`, which is also why the `Crunched` sibling counts 0 in both. The assert needle survives one of the two: `mipLevel < m_MipCount` is **1 in the 2023.1.22f1 editor** and **0 in the 6000.6.2f1 one**, and 0 in every player of both. So the refusal outlives our last hop in a `GraphicsFormat` spelling, and the assert is gone a generation later than that. This literal method - and every claim drawn from it, including the positive statement "the message does not exist in 2018.x" - is bounded to 2022.3.62f3 and earlier, which covers every engine this project has ever hopped to or from; the 2023.1/6000.6 rows are reported for completeness and decide nothing about the ladder.

**What is measured in-engine, and what is not.** `artifacts\decal-texture-probe.log` (written 2026-09-19 15:05:21 by the sibling probe harness, during the run whose raw log is `artifacts\decal-repro.log`) sweeps the constructor directly inside a 2021.3.45f2 editor: `DXT5` **1x1, 2x2 and 3x3 all FAIL** with `UnityException: Failed to create texture because of invalid parameters`, with and without a mip chain, while **4x4 and 8x8 succeed** (`mipmapCount=3` and `4` with a chain, `1` without); `DXT1` behaves identically; `RGBA32` 1x1 succeeds. The mip chain is not the discriminator - the same 1x1 is refused with `mipChain=False` - so the refusal is about the texel size, exactly as the message says. The same run is a clean reproduction of the crash in isolation: `decal-repro.log:1176` prints the multiple-of-four line and `:1191` the exception, 13 times in 1 370 lines. For **2018.1.9f2 / 2018.4.36f1 this is not measured**: no log on this machine ever exercised the plugin on those engines (the 0 `Decal_Maker` above), Unity's own documentation does not state whether the call was accepted, padded or silently mishandled there, and the binaries only say it was not refused with *this* message. Settling it needs a bounded A/B: in the still-installed 2018.4.36f1 editor, and in 2018.1.9f2 if it is present, construct `new Texture2D(w, h, TextureFormat.DXT5, mipChain)` for 1x1, 2x2, 3x3, 4x4 and 8x8 with the chain on and off plus `DXT1` and `RGBA32` controls, and print `width/height/format/mipmapCount` after each - the 2020.3 evidence predicts a non-throwing object followed by a `d3d11: failed … 80070057` line. **That experiment was not run here**; the editor was held by another agent for that turn. It is still unrun, and the verdict below closes the 2018.x half by binary scan instead - which bounds what those engines could have done without settling it by execution.

#### The plugin's own state on this machine

The three package roots are not three copies. `VAMOpen\VaM_Rebuild\AddonPackages` and `VAMOpen\artifacts\player\AddonPackages` are **junctions** into the installation's folder (`Get-Item -Force`: `LinkType=Junction`, `Target=C:\Games\VaM_Updater\AddonPackages`), so one directory is being seen three times - 80 packages, and exactly **two Decal Maker revisions in it**:

| File | Bytes | Written |
| --- | --- | --- |
| `AddonPackages\Chokaphi.DecalMaker.37.var` | 2 867 016 | 2026-09-17 21:07:36 |
| `AddonPackages\Chokaphi.DecalMaker.38.var` | 1 350 901 | 2026-09-19 15:07:02 |

**v38 is not an upstream release and not the cause: it is a local patch, made after the crash, and it is the same defect.** It appeared at 15:07:02, 105 s after the harness wrote `decal-repro.log`/`decal-texture-probe.log` (15:05:21), and it was produced by `scripts\New-DecalMakerPatch.ps1` (9 454 B, written 15:06:15), which copies all 46 entries of v37 byte for byte and rewrites a single line - a fact recorded by the agent that made it, `docs\verification.md:1682-1694`. Both revisions ship one source file, `Custom/Scripts/Chokaphi/VAM_Decal_Maker/VAM_Decal_Maker.cs`, 220 502 B, cs SHA256 `1e0c0886…` in v37 against `ad1abdc2…` in v38; all 46 zip entries have identical sizes in both (measured per entry by `plugin_var_diff.py`, so the packages differ by content and not by payload), and the two sources differ at **exactly two byte offsets, 11662 and 11665**:

```
v37:  texture = new Texture2D(1, 1, TextureFormat.DXT5, linear);
v38:  texture = new Texture2D(4, 4, TextureFormat.DXT5, linear);
```

The string counts around it are identical in both revisions (`GetResource` 19, `ConvertNormal` 2, `UpdateSkinImage` 2, `ManagerPanel` 66, `TextureFormat` 12, `Decal_Maker` 84), so the change is that one literal and not a rewrite - and that literal is the **only** DXT5 construction in the whole 220 502-byte file, which is why one number causes the whole failure (the unpacked copy at `artifacts\decal-maker\…\VAM_Decal_Maker.cs:219`; the file's other constructions are 4 096x4 096 `RGBA32`, a format with no size constraint). Whether the loader picks v38 for a scene that names v37 was **argued here from the version number** - a higher number and no `AddonPackagesUserPrefs` pin - and that argument has since been **measured, and it is wrong**. `FileManager.GetPackage(string packageUidOrPath)` (`src\Assembly-CSharp\FileManager.cs:1560-1592`) consults the package group for exactly two spellings, `^([^\.]+\.[^\.]+)\.latest$` -> `packageGroup.NewestPackage` and `^([^\.]+\.[^\.]+)\.min([0-9]+)$` -> `GetClosestMatchingPackageVersion(requestVersion, false, false)`; every other spelling is a `ContainsKey`/`TryGetValue` on the **exact** uid or path, and the uid of a file inside a package *is* the version-qualified string (`VarFileEntry.Uid = vp.Uid + ":/" + InternalSlashPath`, `VarFileEntry.cs:29`; `VarDirectoryEntry.cs:88`). So only the literal words `.latest` and `.minNN` reach the group; `Chokaphi.DecalMaker.37:/...` addresses revision 37 and nothing else, and a repack under a higher revision is unreferenced **by construction**. The A/B run says the same operationally: the first attempt kept loading `.37` with `.38` on disk. Reaching a scene therefore needs either that scene's version token changed or a genuine rewrite of the shipped `.37`, which is why `scripts\New-DecalMakerPatch.ps1` carries both modes - see the `#### Verdict` below and `docs\decal-texture-crash.md`.

**Which run used what.** Every run whose log survives - the crash and all three 2020.3 baselines - resolved **v37**. The only binding on this machine is the player's scene copy, `artifacts\player\Saves\scene\1789676224.json:893`: `"plugin#2" : "Chokaphi.DecalMaker.37:/Custom/Scripts/Chokaphi/VAM_Decal_Maker/VAM_Decal_Maker.cs"`, and the 2020.3 run unloads the plugin's own asset bundle by that exact revision name - `Unloading unused asset bundle Chokaphi.DecalMaker.37:/Custom/Scripts/Chokaphi/VAM_Decal_Maker/Icons/chokpahi-decal.assetbundle` (`hop4-baseline-ladyclown-smoke.log:7134`), which places `.37` in use in that run too. In the crashing run the scene chain is short and identifiable: `Load Saves/scene/` occurs exactly **once** in 21 618 842 lines, at LF **916** - `Load Saves/scene/MeshedVR/default.json` - with `MVRPluginManager` first at LF **970**, and the failure arrives while a saved character's plugin callback runs inside `SuperController/<LoadCo>d__1235:MoveNext` (`:2923`). The boot scene is not where the plugin comes from. The log also never names the package: `Chokaphi` **0**, `DecalMaker` **0**, `TextureFormat.DXT5` **0** over the whole file - which is why a silently-absent exception proves nothing about whether the plugin loaded, and why this section does not use the absence of a line as evidence.

**How it is attached, and that nothing of it is missing.** It is a **scene plugin on a Person atom** (`plugin#2`, driven by `CharacterUpdated`), not a session plugin: `SessionPlugin` is **0** in the 1.6 GB log and 0 in every log on this machine, and `C:\Games\VaM_Updater\Custom\Scripts\Chokaphi\VAM_Decal_Maker\` holds only `Presets\`/`PreFabs\` subdirectories - the script itself is loaded from inside the `.var`. Our tree is not short of any of it: `C:\Games\VaM_Updater\Custom` and `VAMOpen\VaM_Rebuild\Custom` are **106 files / 1 168 287 164 B each**, identical per file by SHA256 (`artifacts\decal-census\SUMMARY.txt`), and the asset side is clean by measurement rather than assumption - the census reads 3 918 images out of `Custom`, `Saves` and all 79 `.var` packages by header alone and finds 18 whose width or height is not a multiple of 4 and **0 unreadable**, while the Decal Maker's own 41 images are all square powers of two (4 096x4 096 and 8 192x8 192 payloads, 512x512 icons). So the malformed size is not a bad decal texture; it is a literal in the plugin's source.

One more 2021.3-only symptom is recorded because it is new at this hop - and because it must not be mistaken for the cause. `The referenced script (VAM_Decal_Maker.ButtonExt) on this Behaviour is missing!` appears **12** times in the crashing log, first at LF **24991**, inside a 20-line block of the same complaint that also names `MacGruber.SuperShot/PreviewCameraHook` (the first `The referenced script` of all, LF **24988**), `MacGruber.UIDynamicTextInfo` and `VRAdultFun.EmotionEngine` - so the block is not decal-specific, the class it names exists in the very file that throws (`VAM_Decal_Maker.cs:3258`, `public class ButtonExt : Button`), and the string `ButtonExt` occurs in **no other log under 50 MB on this machine** (206 815 `.log`/`.txt` files scanned across `artifacts\`, `logs\` and `AppData\LocalLow\MeshedVR\` - my own script's count, after excluding the census and scratch directories; the only other hit is that source file, 3 times). Measured: the string, its 12 occurrences and its first line. Not measured: why it appears.

#### Verdict

**Engine strictness surfaced a latent plugin bug - our data did not change.** The plugin has always asked for `new Texture2D(1, 1, TextureFormat.DXT5, linear)`; that request is accepted on the engines this project used up to 2020.3.49f1 and refused from 2021.2 on, and our last hop is 2020.3.49f1 -> 2021.3.45f2. Four independent readings agree. *Operationally*, the same plugin frame on the same asset is tolerated under 2020.3.49f1 - the object is constructed, the GPU upload fails with `E_INVALIDARG` and is only logged (`hop4-baseline-ladyclown-smoke.log:1223/1238/1253/1268`, its two twins identical: 72 `Decal_Maker` lines each, 0 refusals, run continues) - and fatal under 2021.3.45f2, where the constructor throws and the coroutine dies (`manual-play-prefix-leak.log:2928-2943`, reproduced in isolation at `decal-repro.log:1176/1191`). *Normatively*, the 2021.2.0 release notes state the restriction and Issue Tracker 6704 states that it was deliberately not backported to 2020.3, describing our own 2020.3 symptom in as many words: *"in Unity 2020.3, DXT-textures with non-multiple-of-four sizes are stored in compressed form, but uploaded to GPU in uncompressed form"*. *Structurally*, the refusal's format string exists in no 2018.1.9f2 or 2018.4.36f1 binary and in every 2020.3, 2021.3 and 2022.3 binary, with positive controls proving the scans. And *on the data side nothing moved*: the three package roots are one junctioned directory, plugin v37 is the revision every run resolved, `Custom\` matches the original installation byte for byte, the plugin's own images are all square powers of two, and the offending size is a literal in the plugin's source - the single line a local patch changes to `4, 4`.

**Both items this verdict first left open are now closed, and the second one was answered in the opposite direction to the claim that stood beside it.** The 2018.x half (item a) is settled by the binary scan above rather than by a run: the refusal's `Compressed ...`/`Crunched ...` format strings exist in **no** binary of 2018.1.9f2 or 2018.4.36f1, editor or player, so those engines cannot have refused this call with this message - which is all the verdict needs of them, although the direct reading, the same constructor sweep run in the installed 2018.4.36f1 editor, is still unrun. Item (b), whether the local v38 patch removes the symptom end to end, **is measured and it passed**, on the same scene with the plugin's version token changed from `.37` to `.38`: `Failed to create texture` **2 -> 0**, the plugin's own `multiple of 4` block **1 -> 0**, `at VAM_Decal_Maker....GetResource` frames **6 -> 0**, `GetResource`/`ConvertNormal` anywhere **2/2 -> 0/0**, `UpdateSkinImage` frames **1 -> 8** and `GetCurrentGPUTexture` frames **0 -> 8** (`artifacts\decal-repro\step2-acceptance.txt`; full record `artifacts\decal-repro\REPORT.md`). The wrinkle is the one the earlier reasoning got wrong: the pass needed the **scene-side version token** too, because `FileManager.GetPackage` resolves a version-qualified url exactly - a v38 package on its own is inert for a scene that names v37, so reaching the scenes that exist needs either that edit per scene or an `-InPlace` rewrite of the shipped `.37`. The verdict is untouched by either item: the 2020.3-against-2021.3 comparison spans the change on its own.
