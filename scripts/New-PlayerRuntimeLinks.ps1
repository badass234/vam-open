...
function Connect-Junction([string]$Link, [string]$Source, [string]$Label, [switch]$Remove) {
    ...
    switch ($state) {
        ...
        'folder' {
            ...
            #
            # One build artifact is neither: since 2020.3, Unity Services (reached through
            # com.unity.purchasing 4.8) writes UnityServicesProjectConfiguration.json into the
            # StreamingAssets directory itself, and to do that the build has to replace the junction
            # with a real directory first. The file is regenerated by every build, the game reads
            # nothing from it, and it is the only thing standing between the build and a runnable
            # player - so it is dropped with the directory instead of stopping the link.
            $leftovers = @(Get-ChildItem -LiteralPath $Link -Recurse -File |
                Where-Object { $_.FullName -ne (Join-Path $Link 'UnityServicesProjectConfiguration.json') })
