...
//
//  Deliberate deviations from the bytecode, all of them cosmetic:
//    * the reflection mip level is a linear ramp of the gloss map instead of
//      the original's exp() curve (same endpoints, sharper falloff in the
//      middle);
//    * the sheen/subdermis terms are added in the fragment stage rather than
//      being folded into the per-vertex colour the original computed;
//    * lightmap indirection (_ExposureLM, unity_ProbeVolume*) is read as a
//      plain ambient scale because the rebuilt project bakes no lightmaps.
//  Everything that decides the pose, the silhouette and the base shading --
//  the structured-buffer skinning, the normals, the diffuse/specular response,
//  the shadow coordinates and the alpha cutoff -- is a direct transcription.
// =============================================================================
...
// The 9 VaM SH coefficients.  VaM publishes _SH0.._SH8 (one direction per
// register) *in addition to* Unity's own probe SH; the decompiled body pixel
// shader uses the VaM set for the sky ambient and the built-in set for the
// light-probe ambient, scaling each with its own exposure.  Their basis order
// is not recoverable from the bytecode, so the ambient below is taken from
// Unity's probe SH -- which is correct by construction -- at the magnitude
// VaM's exposures ask for.
float3 VamAmbientScale() {
    float s = max(_ExposureLM.x, _ExposureIBL.x);
    return s > 0.0 ? s.xxx : float3(1,1,1);
}
...
//  Pixel stage
//
//  Surface terms follow the decompiled body pixel shader: albedo with the
//  decal composited in, two independently perturbed normals (one for the
//  diffuse lobe, one for the specular lobe), a bounded Fresnel, a gloss-driven
//  reflection mip, and an ambient term built from Unity's probe SH at VaM's
//  exposure.  The subdermis colour tints the ambient, which is what gives VaM
//  skin its characteristic translucency.
// -----------------------------------------------------------------------------
struct vam_surface {
    float3 albedo;
    float3 nrmDiff;
...
float VamReflectionScale() {
    return _ExposureIBL.y > 0.0 ? _ExposureIBL.y : 1.0;
}
...
vam_surface VamSurface(vam_v2f i) {
    ...
    albedo = lerp(decal.rgb * decal.a, albedo, 1.0 - decal.a);

    float2 bumpXY = VAM_SAMPLE_BUMP(i.uvBump).xy * 2.0 - 1.0;
    float3 nTS = float3(bumpXY, sqrt(saturate(1.0 - dot(bumpXY, bumpXY))));
    float3 nDiffTS = normalize(nTS + float3(0.0, 0.0, -VAM_DiffuseBumpiness));
    float3 nSpecTS = normalize(nTS + float3(0.0, 0.0, -VAM_SpecularBumpiness));
...
float3 VamShade(vam_v2f i, vam_surface s, float3 V) {
    // ---- Fresnel, bounded the way the original bounds it ---------------------
    float f0 = 1.0 - saturate(dot(s.nrmSpec, V));
    float fres = lerp(pow(f0, 5.0), f0, saturate(VAM_Fresnel));
    fres = fres * 0.95 + 0.05;
    float bound = sqrt(saturate(fres * VAM_SpecInt));
    fres = (fres * VAM_SpecInt - bound) * 0.922444 + bound;

    // ---- Image-based reflection --------------------------------------------
    // The gloss texture drives the mip level: glossy areas (gloss -> 1) read
    // the sharpest mip, rough areas the coarsest.
    float mip = saturate(1.0 - s.gloss) * 8.0;
    float3 R = VamSkyDir(reflect(-V, s.nrmSpec));
    float4 ibl = VAM_SAMPLE_IBL(R, mip);
    float polish = 0.465336 * ibl.w + 25.012255 * ibl.w * ibl.w
                 + 49.174381 * ibl.w * ibl.w * ibl.w;
    float3 reflection = ibl.rgb * polish * s.spec * fres * VamReflectionScale();

    // ---- Ambient / subdermis ------------------------------------------------
    float3 amb = ShadeSH9(float4(s.nrmDiff, 1.0)) * VamAmbientScale();
    float subdermis = saturate(VAM_SubdermisColor.a);
    float3 ambient = amb * lerp(float3(1, 1, 1), VAM_SubdermisColor.rgb, subdermis);
    ...
    float3 direct = _LightColor0.rgb * s.albedo * ndl * atten;

    return ambient + reflection + direct;
}
...
// Additive pass for every light after the main one.  Same surface, but the
// light, its colour and its attenuation all come from the current Unity light.
fixed4 VamFragmentAdd(vam_v2f i) : SV_Target {
    ...
    float3 col = s.albedo * _LightColor0.rgb * ndl * atten;
    return fixed4(col, s.alpha);
}
