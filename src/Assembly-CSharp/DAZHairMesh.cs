...
public class DAZHairMesh : MonoBehaviour
{
	...
	{
		globalSettings.scale = base.transform.lossyScale.x;
		globalSettings.oneoverscale = 1f / base.transform.lossyScale.x;
		globalSettings.drawFromAnchor = drawFromAnchor;
		globalSettings.hairLength = hairLength * globalSettings.scale;
		globalSettings.segmentLength = hairLength * globalSettings.scale / (float)numberSegments;
		globalSettings.quarterSegmentLength = globalSettings.segmentLength * globalSettings.scale * 0.25f;
		globalSettings.hairWidth = hairWidth * globalSettings.scale;
		globalSettings.hairHalfWidth = hairWidth * globalSettings.scale * 0.5f;
		...
		globalSettings.staticMoveDistance = staticMoveDistance * globalSettings.scale;
		globalSettings.staticMoveDistanceSqr = globalSettings.staticMoveDistance * globalSettings.staticMoveDistance;
	...
	private void SetHairStripLiveVars(int i, int j, bool initial)
	{
		...
		root.x = vector.x + vector2.x * anchorOffset * globalSettings.scale;
		root.y = vector.y + vector2.y * anchorOffset * globalSettings.scale;
		root.z = vector.z + vector2.z * anchorOffset * globalSettings.scale;
		hs[i].root = root;
	...
	{
		closeColliders = Physics.OverlapSphere(meshCenter, meshRadius + hairLength * globalSettings.scale, colliderMask);
		int num = 0;
		...
		foreach (Collider collider in array)
		{
			if (collider is SphereCollider)
			{
				...
				closeColliderCenters[num] = collider.transform.TransformPoint(sphereCollider.center);
			}
			else if (collider is BoxCollider)
			{
				BoxCollider boxCollider = (BoxCollider)collider;
				closeColliderCenters[num] = collider.transform.TransformPoint(boxCollider.center);
			}
			else if (collider is CapsuleCollider)
			{
				CapsuleCollider capsuleCollider = (CapsuleCollider)collider;
				closeColliderCenters[num] = collider.transform.TransformPoint(capsuleCollider.center);
			}
			else
			{
				closeColliderCenters[num] = Vector3.zero;
			}
			num++;
			if (num >= 100)
			{
	...
	protected void StartThreads()
	{
		if (!_threadsRunning)
		{
			...
			mainHairTask.resetEvent = new AutoResetEvent(false);
	...
	public void init()
	{
		...
		for (int l = 0; l < _numHairs; l++)
		{
			...
			SetHairStripLiveVars(l, j2, true);
			hs[l].Init();
	...
	private void UpdateHairStripsThreaded()
	{
		...
		for (int i = 0; i < _numHairs; i++)
		{
			...
			if (initCount > 0)
			{
				SetHairStripLiveVars(i, j, true);
			}
			else
			{
				SetHairStripLiveVars(i, j, false);
			}
		}
		if (initCount > 0)
		{
	...
	private void StartFrame()
	{
		...
		foreach (int vertexIndex in vertexIndices)
		{
			...
			smv[vertexIndex] = _scalpSelection.skin.rawSkinnedVerts[vertexIndex];
			smn[vertexIndex] = _scalpSelection.skin.postSkinNormals[vertexIndex];
		}
	}

	private void FinishFrame()
	{
		float num = (meshRadius + hairLength * globalSettings.scale) * 2f;
	...
	private void LateUpdate()
	{
		...
		if (drawHairs)
		{
			if (hairMaterialRuntime.GetFloat("_Scale") != globalSettings.scale)
			{
				hairMaterialRuntime.SetFloat("_Scale", globalSettings.scale);
			}
			for (int i = 0; i < hmverts.Count; i++)
			{
