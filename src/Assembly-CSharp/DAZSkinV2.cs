...
public class DAZSkinV2 : PreCalcMeshProvider, RenderSuspend
{
	...
	public SkinMethod skinMethod = SkinMethod.CPUAndGPU;
	...
	public override Mesh MeshForImport => null;
	...
	public override Color[] VertexSimColors => null;
	...
	public int numSubThreads
	{
		...
		set
		{
			...
			{
				bool subThreadsRunning = _subThreadsRunning;
	...
	public ComputeBuffer surfaceNormalsRawBuffer => _surfaceNormalsRawBuffer;

	public Vector3[] postSkinNormalsThreaded => postSkinWorkingNormals;
	...
	{
		if (needsDispatch && smoothedVertsBuffer != null)
		{
			needsDispatch = false;
			GPUSkinner.SetBuffer(_copyKernel, "inVerts", smoothedVertsBuffer);
			GPUSkinner.SetBuffer(_copyKernel, "outVerts", preCalcVertsBuffer);
			GPUSkinner.Dispatch(_copyKernel, numVertThreadGroups, 1, 1);
			if (provideToWorldMatrices)
			{
				GPUSkinner.SetBuffer(_calcChangeMatricesKernel, "originalPositions", _originalVerticesBuffer);
				GPUSkinner.SetBuffer(_calcChangeMatricesKernel, "originalNormals", _originalNormalsBuffer);
				GPUSkinner.SetBuffer(_calcChangeMatricesKernel, "originalTangents", _originalTangentsBuffer);
				GPUSkinner.SetBuffer(_calcChangeMatricesKernel, "currentPositions", preCalcVertsBuffer);
				GPUSkinner.SetBuffer(_calcChangeMatricesKernel, "currentNormals", _normalsBuffer);
				GPUSkinner.SetBuffer(_calcChangeMatricesKernel, "currentTangents", _tangentsBuffer);
				GPUSkinner.SetBuffer(_calcChangeMatricesKernel, "vertexChangeMatrices", _matricesBuffer);
				GPUSkinner.Dispatch(_calcChangeMatricesKernel, numVertThreadGroups, 1, 1);
			}
		}
	}
	...
	{
		Dictionary<int, DAZSkinV2VertexWeights> value;
		if (boneWeightsMap.TryGetValue(bone.name, out value))
		{
			accumlationStarted = true;
			{
				foreach (Transform item in bone)
				{
					...
					{
						DAZSkinV2VertexWeights value2;
						if (vertexDoneAccumulating.ContainsKey(key) || !dictionary.TryGetValue(key, out value2))
						{
							continue;
						}
						DAZSkinV2VertexWeights value3;
						if (value.TryGetValue(key, out value3))
						{
							...
							if (value3.xweight > 0.99999f && value3.yweight > 0.99999f && value3.zweight > 0.99999f)
							{
								...
								value3.yweight = 1f;
								value3.zweight = 1f;
								vertexDoneAccumulating.Add(key, true);
							}
							value.Remove(key);
	...
	protected void CreateBoneWeightsArray()
	{
		foreach (string key in boneWeightsMap.Keys)
		{
			Dictionary<int, DAZSkinV2VertexWeights> value;
			int value2;
			if (!boneWeightsMap.TryGetValue(key, out value) || !boneNameToIndexMap.TryGetValue(key, out value2))
			{
				continue;
			}
			...
			{
				DAZSkinV2VertexWeights value3;
				if (value.TryGetValue(key2, out value3))
				{
		...
		{
			Dictionary<int, DAZSkinV2GeneralVertexWeights> value4;
			if (!boneGeneralWeightsMap.TryGetValue(key3, out value4))
			{
				continue;
			}
			int num = 0;
			int value5;
			if (!boneNameToIndexMap.TryGetValue(key3, out value5))
			{
				continue;
			}
			...
			{
				DAZSkinV2GeneralVertexWeights value6;
				if (value4.TryGetValue(key4, out value6))
				{
	...
	public void Import(JSONNode jn)
	{
		...
		foreach (JSONNode item in jSONNode.AsArray)
		{
			...
			foreach (JSONNode item2 in item["node_weights"]["values"].AsArray)
			{
				...
				List<int> value;
				if (!baseVertToUVVertFullMap.TryGetValue(asInt, out value))
				{
					continue;
				}
				foreach (int item3 in value)
				{
					DAZSkinV2GeneralVertexWeights value2;
					if (dictionary2.TryGetValue(item3, out value2))
					{
			...
			foreach (JSONNode item4 in item["local_weights"]["x"]["values"].AsArray)
			{
				...
				List<int> value3;
				if (!baseVertToUVVertFullMap.TryGetValue(asInt2, out value3))
				{
					continue;
				}
				foreach (int item5 in value3)
				{
					DAZSkinV2VertexWeights value4;
					if (dictionary.TryGetValue(item5, out value4))
					{
			...
			foreach (JSONNode item6 in item["local_weights"]["y"]["values"].AsArray)
			{
				...
				List<int> value5;
				if (!baseVertToUVVertFullMap.TryGetValue(asInt3, out value5))
				{
					continue;
				}
				foreach (int item7 in value5)
				{
					DAZSkinV2VertexWeights value6;
					if (dictionary.TryGetValue(item7, out value6))
					{
			...
			foreach (JSONNode item8 in item["local_weights"]["z"]["values"].AsArray)
			{
				...
				List<int> value7;
				if (!baseVertToUVVertFullMap.TryGetValue(asInt4, out value7))
				{
					continue;
				}
				foreach (int item9 in value7)
				{
					DAZSkinV2VertexWeights value8;
					if (dictionary.TryGetValue(item9, out value8))
					{
			...
			foreach (JSONNode item11 in item["bulge_weights"]["x"]["left_map"]["values"].AsArray)
			{
				...
				List<int> value9;
				if (!baseVertToUVVertFullMap.TryGetValue(asInt5, out value9))
				{
					continue;
				}
				foreach (int item12 in value9)
				{
					DAZSkinV2VertexWeights value10;
					if (dictionary.TryGetValue(item12, out value10))
					{
			...
			foreach (JSONNode item13 in item["bulge_weights"]["x"]["right_map"]["values"].AsArray)
			{
				...
				List<int> value11;
				if (!baseVertToUVVertFullMap.TryGetValue(asInt6, out value11))
				{
					continue;
				}
				foreach (int item14 in value11)
				{
					DAZSkinV2VertexWeights value12;
					if (dictionary.TryGetValue(item14, out value12))
					{
			...
			foreach (JSONNode item16 in item["bulge_weights"]["y"]["left_map"]["values"].AsArray)
			{
				...
				List<int> value13;
				if (!baseVertToUVVertFullMap.TryGetValue(asInt7, out value13))
				{
					continue;
				}
				foreach (int item17 in value13)
				{
					DAZSkinV2VertexWeights value14;
					if (dictionary.TryGetValue(item17, out value14))
					{
			...
			foreach (JSONNode item18 in item["bulge_weights"]["y"]["right_map"]["values"].AsArray)
			{
				...
				List<int> value15;
				if (!baseVertToUVVertFullMap.TryGetValue(asInt8, out value15))
				{
					continue;
				}
				foreach (int item19 in value15)
				{
					DAZSkinV2VertexWeights value16;
					if (dictionary.TryGetValue(item19, out value16))
					{
			...
			foreach (JSONNode item21 in item["bulge_weights"]["z"]["left_map"]["values"].AsArray)
			{
				...
				List<int> value17;
				if (!baseVertToUVVertFullMap.TryGetValue(asInt9, out value17))
				{
					continue;
				}
				foreach (int item22 in value17)
				{
					DAZSkinV2VertexWeights value18;
					if (dictionary.TryGetValue(item22, out value18))
					{
			...
			foreach (JSONNode item23 in item["bulge_weights"]["z"]["right_map"]["values"].AsArray)
			{
				...
				List<int> value19;
				if (!baseVertToUVVertFullMap.TryGetValue(asInt10, out value19))
				{
					continue;
				}
				foreach (int item24 in value19)
				{
					DAZSkinV2VertexWeights value20;
					if (dictionary.TryGetValue(item24, out value20))
					{
	...
	protected void InitRecalcNormalsTangents()
	{
		...
		{
			recalcNormals = new RecalculateNormals(dazMesh.baseTriangles, workingVerts2, workingNormals, workingSurfaceNormals, true);
		}
		if (postSkinRecalcNormals == null)
		{
			postSkinRecalcNormals = new RecalculateNormals(dazMesh.baseTriangles, rawSkinnedWorkingVerts, postSkinWorkingNormals, postSkinWorkingSurfaceNormals);
		}
		if (skinMethod != 0)
		{
			if (originalRecalcNormalsGPU == null)
			{
				originalRecalcNormalsGPU = new RecalculateNormalsGPU(GPUMeshCompute, dazMesh.baseTriangles, numUVVerts, dazMesh.baseVerticesToUVVertices);
				...
				originalRecalcNormalsGPU.RecalculateNormals(_originalVerticesBuffer);
			}
			...
			if (recalcNormalsGPURaw == null)
			{
				recalcNormalsGPURaw = new RecalculateNormalsGPU(GPUMeshCompute, dazMesh.baseTriangles, numUVVerts, dazMesh.baseVerticesToUVVertices);
				_surfaceNormalsRawBuffer = recalcNormalsGPURaw.surfaceNormalsBuffer;
			}
		}
		if (_recalculateTangents && recalcTangents == null)
		{
			recalcTangents = new RecalculateTangents(dazMesh.UVTriangles, workingVerts2, workingNormals, dazMesh.UV, workingTangents, true);
		}
		if (skinMethod != 0)
		{
			if (originalRecalcTangentsGPU == null)
			{
				originalRecalcTangentsGPU = new RecalculateTangentsGPU(GPUMeshCompute, dazMesh.UVTriangles, dazMesh.UV, numUVVerts);
				_originalTangentsBuffer = originalRecalcTangentsGPU.tangentsBuffer;
				originalRecalcTangentsGPU.RecalculateTangents(_originalVerticesBuffer, _originalNormalsBuffer);
			}
	...
	protected void InitMesh()
	{
		...
		postSkinVertsReady = new bool[numUVVerts];
		...
		workingVerts1 = (Vector3[])startVerts.Clone();
		workingVerts = workingVerts1;
		...
		postSkinWorkingSurfaceNormals = new Vector3[dazMesh.baseTriangles.Length / 3];
	...
	{
		if (_subThreadsRunning)
		{
			return;
		}
		_subThreadsRunning = true;
		if (_numSubThreads > 0)
		{
	...
	protected void StartThreads()
	{
		if (!_threadsRunning)
		{
			...
			normalTangentTask.resetEvent = new AutoResetEvent(false);
			...
			mainSkinTask.resetEvent = new AutoResetEvent(false);
			...
			postSkinMorphTask.resetEvent = new AutoResetEvent(false);
	...
	protected void MTTask(object info)
	{
		...
		while (_threadsRunning || _subThreadsRunning)
		{
			...
			{
				threadSkinStartTime[dAZSkinTaskInfo.threadIndex] = (float)stopwatch.ElapsedTicks * f;
				SkinMeshPart(dAZSkinTaskInfo.index1, dAZSkinTaskInfo.index2, postSkinNeededVerts, postSkinBones);
				threadSkinStopTime[dAZSkinTaskInfo.threadIndex] = (float)stopwatch.ElapsedTicks * f;
				threadSkinTime[dAZSkinTaskInfo.threadIndex] = threadSkinStopTime[dAZSkinTaskInfo.threadIndex] - threadSkinStartTime[dAZSkinTaskInfo.threadIndex];
			}
	...
	protected void NormalTangentRecalc()
	{
		if (_recalculateNormals)
		{
			...
			{
				workingNormals[dAZVertexMap.tovert] = workingNormals[dAZVertexMap.fromvert];
			}
		}
	...
	protected void SkinMeshGPUInit()
	{
		...
		_calcChangeMatricesKernel = GPUSkinner.FindKernel("SkinWrapCalcChangeMatrices");
		...
		preCalcVertsBuffer = new ComputeBuffer(num + 1, 12);
		...
		_originalVerticesBuffer = new ComputeBuffer(num + 1, 12);
		...
		PreCalculatedVerticesBuffer = new GpuBuffer<Vector3>(preCalcVertsBuffer);
	...
	protected void SkinMeshGPUCleanup()
	{
		...
		if (recalcNormalsGPURaw != null)
		{
			recalcNormalsGPURaw.Release();
			recalcNormalsGPURaw = null;
		}
		if (originalRecalcTangentsGPU != null)
		{
		...
		if (preCalcVertsBuffer != null)
		{
			preCalcVertsBuffer.Release();
			preCalcVertsBuffer = null;
		}
	...
	protected void RecalculatePostSkinNeededVertsAndTriangles()
	{
		...
		postSkinNeededTriangleIndices = list2.ToArray();
		postSkinNeededVertsList = list3.ToArray();
	}
	...
	public void SkinMeshGPUPostSkinVertsStart()
	{
		if (_useGeneralWeights)
		{
			for (int i = 0; i < postSkinNeededVertsList.Length; i++)
			{
				...
				startVertsCopy[num] = startVerts[num];
		...
		else
		{
			for (int j = 0; j < postSkinNeededVertsList.Length; j++)
			{
				...
				workingVerts[num2] = startVerts[num2];
			}
		}
	...
	public void SkinMeshGPUPostSkinVertsThreaded()
	{
		...
		{
			int num = postSkinNeededVertsList[i];
			rawSkinnedWorkingVerts[num] = workingVerts[num];
		}
	...
	{
		postSkinRecalcNormals.recalculateNormals(vertsToUse);
		DAZVertexMap[] array = baseVertsToUVVertsCopy;
		foreach (DAZVertexMap dAZVertexMap in array)
		{
			postSkinWorkingNormals[dAZVertexMap.tovert] = postSkinWorkingNormals[dAZVertexMap.fromvert];
		}
		return (Vector3[])postSkinWorkingNormals.Clone();
	}

	public void SmoothAllVertices(Vector3[] inVerts, Vector3[] outVerts)
	{
		meshSmooth.LaplacianSmooth(inVerts, outVerts);
		meshSmooth.HCCorrection(inVerts, outVerts, laplacianSmoothBeta);
		DAZVertexMap[] array = baseVertsToUVVertsCopy;
		foreach (DAZVertexMap dAZVertexMap in array)
		{
	...
	public void SkinMeshGPUPostSkinVertsFinish()
	{
		...
		{
			int num = postSkinNeededVertsList[i];
			postSkinVertsReady[num] = true;
			rawSkinnedVerts[num] = rawSkinnedWorkingVerts[num];
			postSkinNormals[num] = postSkinWorkingNormals[num];
		}
	}
	...
	protected void SkinMeshGPUFrame()
	{
		...
		for (int i = 0; i < numBones; i++)
		{
			...
			boneChangeFromOriginalMatrices[i] = dAZBone.changeFromOriginalMatrix;
			if (!_useGeneralWeights)
			{
				...
				boneWorldToLocalMatrices[i] = dAZBone.morphedWorldToLocalMatrix;
				boneLocalToWorldMatrices[i] = dAZBone.morphedLocalToWorldMatrix;
				if (angles.x >= 0f)
				{
		...
		if (mesh != null && root != null && GPUSkinner != null)
		{
			...
			if (_recalculateNormals)
			{
				if (_useSmoothing && useSmoothVertsForNormalTangentRecalc)
				{
					...
					recalcNormalsGPURaw.RecalculateSurfaceNormals(rawVertsBuffer);
				}
				else
				{
	...
	public void SkinMeshThreadedFast(Vector3[] verts, bool forceSynchronous = false)
	{
		...
		{
			workingVerts[dAZVertexMap.tovert] = workingVerts[dAZVertexMap.fromvert];
		}
	...
	public void SkinMeshPostVertsThreadedFast(Vector3[] verts)
	{
		...
		if (useMultithreadedSkinning && _numSubThreads > 0)
		{
			int num2 = (numUVVerts - numUVOnlyVerts) / _numSubThreads;
			for (int j = 0; j < _numSubThreads; j++)
			{
				tasks[j].taskType = DAZSkinTaskType.PostSkin;
				tasks[j].index1 = j * num2;
				if (j == _numSubThreads - 1)
				{
					tasks[j].index2 = num - 1;
		...
		for (int l = 0; l < postSkinNeededVertsList.Length; l++)
		{
			...
			rawSkinnedWorkingVerts[num3] = workingVerts[num3];
		}
		if (needsPostSkinNormals)
		{
			threadRecalcNormalTangentStartTime = (float)stopwatch.ElapsedTicks * f;
	...
	{
		if (useAsynchronousThreadedSkinning && dazMesh is DAZMergedMesh)
		{
			DAZMergedMesh dAZMergedMesh = dazMesh as DAZMergedMesh;
			dAZMergedMesh.UpdateVerticesPost();
		}
		if (allowPostSkinMorph)
		{
			for (int i = 0; i < postSkinNeededVertsList.Length; i++)
	...
	protected void SkinMeshCPUandGPU(bool forceSynchronous = false)
	{
		...
		if (mesh != null && root != null && GPUSkinner != null)
		{
			...
			{
				RecalcBones();
				SkinMeshStartFrame(forceSynchronous);
	...
	{
		if (useAsynchronousThreadedSkinning && !forceSynchronous && skinMethod != SkinMethod.GPU && dazMesh is DAZMergedMesh)
		{
			dazMesh.enabled = false;
			DAZMergedMesh dAZMergedMesh = dazMesh as DAZMergedMesh;
			startVerts = dAZMergedMesh.threadedMorphedUVVertices;
			workingVerts = startVerts;
			dAZMergedMesh.UpdateVerticesPre(true);
			if (_useGeneralWeights)
			{
				for (int i = 0; i < numBaseVerts; i++)
				{
					startVertsCopy[i] = startVerts[i];
				}
			}
		}
		else
		{
			dazMesh.enabled = true;
			startVerts = dazMesh.morphedUVVertices;
			workingVerts = workingVerts1;
			if (_useGeneralWeights)
			{
				...
				{
					startVertsCopy[j] = startVerts[j];
				}
			}
			else
			{
				for (int k = 0; k < numBaseVerts; k++)
				{
					workingVerts[k] = startVerts[k];
				}
			}
		}
		postSkinVertsChangedThreaded = postSkinVertsChanged;
	...
	{
		if (useAsynchronousThreadedSkinning && skinMethod != SkinMethod.GPU && dazMesh is DAZMergedMesh)
		{
			DAZMergedMesh dAZMergedMesh = dazMesh as DAZMergedMesh;
			dAZMergedMesh.UpdateVerticesPost();
		}
		...
		if (_useSmoothing)
		{
			...
			else
			{
				for (int k = 0; k < numUVVerts; k++)
				{
					drawVerts[k] = smoothedVerts[k];
					unsmoothedVerts[k] = workingVerts[k];
				}
			}
		}
		else
	...
	public void RecalcBones()
	{
		for (int i = 0; i < numBones; i++)
		{
			...
			if (NaNUtils.IsMatrixValid(dAZBone.morphedLocalToWorldMatrix) && NaNUtils.IsMatrixValid(dAZBone.morphedWorldToLocalMatrix) && NaNUtils.IsMatrixValid(dAZBone.changeFromOriginalMatrix))
			{
				...
				if (!_useGeneralWeights)
				{
					boneRotationAngles[i] = dAZBone.GetAngles();
				}
			}
			else
			{
		...
		if (mesh != null && root != null)
		{
			...
			if (!forceSynchronous && !normalTangentTask.working && useAsynchronousNormalTangentRecalc)
			{
				if (_recalculateNormals)
				{
					for (int i = 0; i < numUVVerts; i++)
					{
						drawNormals[i] = workingNormals[i];
					}
					for (int j = 0; j < workingSurfaceNormals.Length; j++)
					{
						drawSurfaceNormals[j] = workingSurfaceNormals[j];
					}
				...
				if (_recalculateTangents)
				{
					for (int k = 0; k < numUVVerts; k++)
					{
						drawTangents[k] = workingTangents[k];
					}
				...
				if (_recalculateNormals || _recalculateTangents)
				{
					...
					else
					{
						...
						{
							workingVerts2[m] = unsmoothedVerts[m];
						}
					}
	...
	protected void SkinMeshThreaded(bool forceSynchronous = false)
	{
		...
		{
			DAZMergedMesh dAZMergedMesh = dazMesh as DAZMergedMesh;
			dAZMergedMesh.UpdateVerticesThreaded();
		}
		int num = numBaseVerts;
		if (_useGeneralWeights)
		{
		...
		if (allowPostSkinMorph)
		{
			if (skinMethod == SkinMethod.CPU)
			{
				for (int l = 0; l < postSkinNeededVertsList.Length; l++)
				{
					int num3 = postSkinNeededVertsList[l];
					rawSkinnedWorkingVerts[num3] = workingVerts[num3];
					workingVerts[num3] += postSkinMorphs[num3];
				}
			}
			else
			{
				for (int m = 0; m < postSkinNeededVertsList.Length; m++)
				{
					int num4 = postSkinNeededVertsList[m];
					rawSkinnedWorkingVerts[num4] = workingVerts[num4];
				}
			}
		}
		if (needsPostSkinNormals)
		...
		if (_useSmoothing && skinMethod == SkinMethod.CPU)
		{
			...
			{
				int num5 = numBaseVerts / _numSubThreads;
				...
				for (int num7 = 0; num7 < _numSubThreads; num7++)
				{
					...
					{
						tasks[num7].index2 = numBaseVerts - 1;
					}
					else
					{
				...
				foreach (DAZVertexMap dAZVertexMap in baseVerticesToUVVertices)
				{
					smoothedVerts[dAZVertexMap.tovert] = smoothedVerts[dAZVertexMap.fromvert];
				}
			}
			else
			{
				...
				{
					smoothedVerts[dAZVertexMap2.tovert] = smoothedVerts[dAZVertexMap2.fromvert];
				}
			}
		}
		else
		{
			...
			{
				workingVerts[dAZVertexMap3.tovert] = workingVerts[dAZVertexMap3.fromvert];
			}
		}
		threadMainSkinStopTime = (float)stopwatch.ElapsedTicks * f;
		threadMainSkinTime = threadMainSkinStopTime - threadMainSkinStartTime;
	...
	protected void DrawMesh()
	{
		...
		if (!alreadyCheckedForMeshComponents)
		{
			meshFilter = GetComponent<MeshFilter>();
			meshRenderer = GetComponent<MeshRenderer>();
			alreadyCheckedForMeshComponents = Application.isPlaying;
		}
		...
		else
		{
			...
			for (int i = 0; i < mesh.subMeshCount; i++)
			{
				if (dazMesh.materialsEnabled[i])
				{
					...
					if (dazMesh.materials != null)
					{
						material = dazMesh.materials[i];
					}
					if (material == null || dazMesh.useSimpleMaterial)
					{
						material = dazMesh.simpleMaterial;
					}
					if (material != null)
					{
						Graphics.DrawMesh(mesh, identity, material, base.gameObject.layer, null, i, null, materialsShadowCastEnabled[i], dazMesh.receiveShadows);
					}
				}
			}
		}
	...
	protected void DrawMeshNative()
	{
		...
		for (int i = 0; i < mesh.subMeshCount; i++)
		{
			if (dazMesh.materialsEnabled[i])
			{
				...
				{
					meshRenderer.materials[i] = dazMesh.simpleMaterial;
				}
				else
				{
					meshRenderer.materials[i] = dazMesh.materials[i];
				}
			}
			else
			{
	...
	public void DrawMeshGPU()
	{
		...
		else
		{
			...
			identity2.m03 += drawOffset.x;
			identity2.m13 += drawOffset.y;
			identity2.m23 += drawOffset.z;
			for (int i = 0; i < mesh.subMeshCount; i++)
			{
				...
				if (GPUuseSimpleMaterial && (bool)GPUsimpleMaterial)
				{
					...
					Graphics.DrawMesh(mesh, identity2, GPUsimpleMaterial, base.gameObject.layer, null, i, null, materialsShadowCastEnabled[i], dazMesh.receiveShadows);
				}
				else
				{
					if (!(GPUmaterials[i] != null))
					...
					{
						ComputeBuffer value;
						materialVertsBuffers.TryGetValue(i, out value);
						if (_useSmoothing)
						{
							if (value != smoothedVertsBuffer)
							{
								GPUmaterials[i].SetBuffer("verts", smoothedVertsBuffer);
								materialVertsBuffers.Remove(i);
								materialVertsBuffers.Add(i, smoothedVertsBuffer);
							}
						}
						else if (value != rawVertsBuffer)
						{
							GPUmaterials[i].SetBuffer("verts", rawVertsBuffer);
							materialVertsBuffers.Remove(i);
							materialVertsBuffers.Add(i, rawVertsBuffer);
						}
						if (_normalsBuffer != null)
						{
							materialNormalsBuffers.TryGetValue(i, out value);
							if (value != _normalsBuffer)
							{
					...
					Graphics.DrawMesh(mesh, identity2, GPUmaterials[i], base.gameObject.layer, null, i, null, materialsShadowCastEnabled[i], dazMesh.receiveShadows);
				}
			}
		}
	}
	...
	public void ResetPostSkinMorphs()
	{
		...
		{
			postSkinNeededVertsList = new int[0];
			postSkinNeededTriangles = new int[0];
			for (int i = 0; i < postSkinMorphs.Length; i++)
			{
				postSkinVerts[i] = false;
				postSkinVertsReady[i] = false;
				postSkinNeededVerts[i] = false;
				postSkinNormalVerts[i] = false;
				postSkinMorphs[i].x = 0f;
				postSkinMorphs[i].y = 0f;
				postSkinMorphs[i].z = 0f;
			}
		}
	}
