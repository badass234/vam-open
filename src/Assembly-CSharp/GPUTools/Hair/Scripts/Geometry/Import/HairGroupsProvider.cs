...
namespace GPUTools.Hair.Scripts.Geometry.Import
{
	...
	public class HairGroupsProvider
	{
		...
		{
			return HairFilters.Select((MeshFilter filter) => MeshUtils.GetVerticesInSpace(filter.sharedMesh, filter.transform.localToWorldMatrix, worldToObject)).ToList();
		}

		private List<Vector3> InitVertices()
		{
			return VerticesGroups.SelectMany((List<Vector3> verticesGroup) => verticesGroup).ToList();
		}

		private List<List<Color>> InitColorGroups()
		{
			return HairFilters.Select((MeshFilter filter) => filter.sharedMesh.colors.ToList()).ToList();
		}

		private List<Color> InitColors()
		{
			return ColorsGroups.SelectMany((List<Color> colorsGroup) => colorsGroup).ToList();
		}
