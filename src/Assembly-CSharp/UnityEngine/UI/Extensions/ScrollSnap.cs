...
namespace UnityEngine.UI.Extensions
{
	...
	public class ScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollSnap, IEventSystemHandler
	{
		...
		public void UpdateListItemsSize()
		{
			...
			if (direction == ScrollDirection.Horizontal)
			{
				foreach (object item in _listContainerTransform)
				{
			...
			foreach (object item2 in _listContainerTransform)
			{
		...
		public void ResetPage()
		{
			if (direction == ScrollDirection.Horizontal)
			{
				_scroll_rect.horizontalNormalizedPosition = ((_pages <= 1) ? 0f : ((float)_startingPage / (float)(_pages - 1)));
			}
			else
			{
				_scroll_rect.verticalNormalizedPosition = ((_pages <= 1) ? 0f : ((float)(_pages - _startingPage - 1) / (float)(_pages - 1)));
			}
		}
		...
		public int CurrentPage()
		{
			...
			float f = value / _itemSize;
			return Mathf.Clamp(Mathf.RoundToInt(f), 0, _pages);
		}
		...
		public void OnEndDrag(PointerEventData eventData)
		{
			_startDrag = true;
			float num = 0f;
			num = ((direction != 0) ? (0f - _positionOnDragStart.y + eventData.position.y) : (_positionOnDragStart.x - eventData.position.x));
			if (UseFastSwipe)
			{
				fastSwipe = false;
