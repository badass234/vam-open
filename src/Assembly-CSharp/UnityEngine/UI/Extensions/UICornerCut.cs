namespace UnityEngine.UI.Extensions
{
	...
	public class UICornerCut : UIPrimitiveBase
	{
		...
		protected override void OnPopulateMesh(VertexHelper vh)
		{
			...
			if (m_makeColumns)
			{
				vector = new Vector2(rect.xMin, (!m_cutUL) ? rect.yMax : rect2.yMax);
				vector2 = new Vector2(rect.xMax, (!m_cutUR) ? rect.yMax : rect2.yMax);
				vector3 = new Vector2(rect.xMin, (!m_cutLL) ? rect.yMin : rect2.yMin);
				vector4 = new Vector2(rect.xMax, (!m_cutLR) ? rect.yMin : rect2.yMin);
				if (flag3)
				{
					AddSquare(vector3, vector, new Vector2(rect2.xMin, rect.yMax), new Vector2(rect2.xMin, rect.yMin), rect, (!m_useColorUp) ? color : m_colorUp, vh);
				}
				if (flag4)
				{
					AddSquare(vector2, vector4, new Vector2(rect2.xMax, rect.yMin), new Vector2(rect2.xMax, rect.yMax), rect, (!m_useColorDown) ? color : m_colorDown, vh);
				}
			}
			else
			{
				vector = new Vector2((!m_cutUL) ? rect.xMin : rect2.xMin, rect.yMax);
				vector2 = new Vector2((!m_cutUR) ? rect.xMax : rect2.xMax, rect.yMax);
				vector3 = new Vector2((!m_cutLL) ? rect.xMin : rect2.xMin, rect.yMin);
				vector4 = new Vector2((!m_cutLR) ? rect.xMax : rect2.xMax, rect.yMin);
				if (flag2)
				{
					AddSquare(vector4, vector3, new Vector2(rect.xMin, rect2.yMin), new Vector2(rect.xMax, rect2.yMin), rect, (!m_useColorDown) ? color : m_colorDown, vh);
				}
				if (flag)
				{
					AddSquare(vector, vector2, new Vector2(rect.xMax, rect2.yMax), new Vector2(rect.xMin, rect2.yMax), rect, (!m_useColorUp) ? color : m_colorUp, vh);
				}
			}
			if (m_makeColumns)
			{
