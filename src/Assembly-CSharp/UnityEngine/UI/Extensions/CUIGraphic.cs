...
namespace UnityEngine.UI.Extensions
{
	...
	public class CUIGraphic : BaseMeshEffect
	{
		...
		protected UIVertex uiVertexLerp(UIVertex _a, UIVertex _b, float _time)
		{
			...
			result.position = Vector3.Lerp(_a.position, _b.position, _time);
			result.normal = Vector3.Lerp(_a.normal, _b.normal, _time);
			result.tangent = Vector3.Lerp(_a.tangent, _b.tangent, _time);
			result.uv0 = Vector2.Lerp(_a.uv0, _b.uv0, _time);
			result.uv1 = Vector2.Lerp(_a.uv1, _b.uv1, _time);
			result.color = Color.Lerp(_a.color, _b.color, _time);
			return result;
		}
		...
		public void ReferenceCUIForBCurves()
		{
			...
			for (int i = 0; i < refCurves.Length; i++)
			{
				...
				Vector3 _x;
				Vector3 _y;
				solveDoubleEquationWithVector(x_, y_, x_2, y_2, constant_, contant_, out _x, out _y);
				cUIBezierCurve.ControlPoints[1] = _x - rectTrans.localPosition;
				cUIBezierCurve.ControlPoints[2] = _y - rectTrans.localPosition;
			}
		}
