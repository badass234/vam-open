...
public class DAZSkin : MonoBehaviour
{
	...
	{
		Dictionary<int, DAZMeshVertexWeights> value;
	...
	public void Import(JSONNode jn)
	{
		...
		foreach (JSONNode item in jSONNode.AsArray)
		{
			...
			foreach (JSONNode item2 in item["node_weights"]["values"].AsArray)
			{
				...
				{
					DAZMeshVertexWeights value2;
					if (dictionary.TryGetValue(item3, out value2))
					{
	...
	protected void InitBones()
	{
		...
		for (int j = 0; j < numBones; j++)
		{
			...
			if (dictionary2.TryGetValue(text, out value2))
			{
				...
				startingMatricesInverted[j] = Matrix4x4.Inverse(value2);
			}
			else
			{
				UnityEngine.Debug.LogError("Could not find transform for bone " + text);
	...
	protected void SkinMesh()
	{
		...
		else
		{
			...
			{
				drawVerts[j] = startVerts[j];
				drawNormals[j] = startNormals[j];
				drawTangents[j] = startTangents[j];
			}
		}
		for (int k = 0; k < numBones; k++)
		{
			Transform transform = boneTransforms[k];
			boneMatrices[k] = transform.localToWorldMatrix * startingMatrices[k];
			if (!_useGeneralWeights)
			{
				DAZBone dAZBone = dazBones[k];
				if (dAZBone == null)
				{
					boneRotationAngles[k] = Quaternion2Angles.GetAngles(transform.localRotation, nodes[k].rotationOrder);
				}
				else
				{
					boneRotationAngles[k] = dAZBone.GetAngles();
				}
			}
		}
		if (tasks == null)
		...
		else
		{
			...
			{
				drawVerts[dAZVertexMap3.tovert] = drawVerts[dAZVertexMap3.fromvert];
			}
