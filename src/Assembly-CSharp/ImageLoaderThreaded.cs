		...
		public void Process()
		{
			...
			if (imgPath != null && imgPath != "NULL")
			{
				...
				else if (FileManager.FileExists(imgPath))
				{
					...
					if (CacheManager.CachingEnabled && diskCachePath != null && FileManager.FileExists(diskCachePath))
					{
						try
						{
	...
	protected void ReportMemory(List<string> reports)
	{
		...
		if (textureCache != null)
		{
			foreach (Texture2D value in textureCache.Values)
			{
				...
				{
					num2 += Profiler.GetRuntimeMemorySizeLong(value);
				}
			}
		}
		...
		{
			foreach (Texture2D value2 in thumbnailCache.Values)
			{
				num3++;
				if (Debug.isDebugBuild)
				{
					num4 += Profiler.GetRuntimeMemorySizeLong(value2);
				}
			}
		}
