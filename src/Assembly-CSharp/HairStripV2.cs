...
public class HairStripV2
{
	...
	{
		hmverts[index1] = p2;
		hmnormals[index1] = anchorToRoot;
		if (globalSettings.createTangents)
		{
			hmtangents[index1] = anchorTangent;
		}
		if (debug)
		{
	...
	private void CreateMeshDataFromPoints(int vindex, int tindex)
	{
		...
		for (int i = 1; i < numHairs; i++)
		{
			...
			float num5 = subHairXYZOffsets[i].x * globalSettings.scale;
			float num6 = subHairXYZOffsets[i].y * globalSettings.scale;
			float num7 = subHairXYZOffsets[i].z * globalSettings.scale;
			if (num5 != 0f)
			{
				zero.x += anchorTangent.x * num5;
				zero.y += anchorTangent.y * num5;
				zero.z += anchorTangent.z * num5;
			}
			if (num6 != 0f)
			{
				zero.x += anchorTangent2.x * num6;
				zero.y += anchorTangent2.y * num6;
				zero.z += anchorTangent2.z * num6;
			}
			if (num7 != 0f)
			{
				zero.x += anchorToRoot.x * num7;
				zero.y += anchorToRoot.y * num7;
				zero.z += anchorToRoot.z * num7;
			}
			for (int j = num; j < num + numVerticesPerHair; j++)
			{
				...
				hmnormals[vindex] = hmnormals[j];
				if (globalSettings.createTangents)
				{
					hmtangents[vindex] = hmtangents[j];
				}
				hmuvs[vindex] = hmuvs[j];
				vindex++;
			}
		}
		for (int k = 1; k < numHairs; k++)
		{
			int num8 = numVerticesPerHair * k;
			for (int l = num2; l < num2 + numTrianglePointsPerHair; l++)
			{
				hmtriangles[tindex] = hmtriangles[l] + num8;
				tindex++;
			}
		}
	}
	...
	private void UpdateMeshDataFromPoints(int vindex)
	{
		...
		if (globalSettings.hairDrawType == HairDrawType.Tube)
		{
			...
			{
				hmverts[vindex] = Vector3.zero;
				hmverts[vindex + 1] = Vector3.zero;
				hmverts[vindex + 2] = Vector3.zero;
				hmverts[vindex + 3] = Vector3.zero;
			}
			vindex += 4;
		}
		else if (globalSettings.hairDrawType == HairDrawType.Sheet)
		...
		{
			Vector3 zero = Vector3.zero;
			float num2 = subHairXYZOffsets[i].x * globalSettings.scale;
			float num3 = subHairXYZOffsets[i].y * globalSettings.scale;
			float num4 = subHairXYZOffsets[i].z * globalSettings.scale;
			if (num2 != 0f)
			{
				zero.x += anchorTangent.x * num2;
				zero.y += anchorTangent.y * num2;
				zero.z += anchorTangent.z * num2;
			}
			if (num3 != 0f)
			{
				zero.x += anchorTangent2.x * num3;
				zero.y += anchorTangent2.y * num3;
				zero.z += anchorTangent2.z * num3;
			}
			if (num4 != 0f)
			{
				zero.x += anchorToRoot.x * num4;
				zero.y += anchorToRoot.y * num4;
				zero.z += anchorToRoot.z * num4;
			}
			if (debug)
			{
	...
	private void MoveThisPointPhysically(float stiffnessGraded)
	{
		...
		float num = globalSettings.deltaTimeSqr * globalSettings.scale;
		float num2 = globalSettings.velocityFactor * globalSettings.deltaTime;
		thisPoint.unconstrained_position.x = thisPoint.previous_position.x + num2 * thisPoint.velocity.x + num * (thisPoint.force.x + globalSettings.gravityForce.x);
		thisPoint.unconstrained_position.y = thisPoint.previous_position.y + num2 * thisPoint.velocity.y + num * (thisPoint.force.y + globalSettings.gravityForce.y);
		thisPoint.unconstrained_position.z = thisPoint.previous_position.z + num2 * thisPoint.velocity.z + num * (thisPoint.force.z + globalSettings.gravityForce.z);
		...
		if (globalSettings.enableCollision)
		{
			...
			else
			{
				...
				foreach (Collider collider in colliders)
				{
					...
					{
						MoveThisPointToColliderSurface(num, false);
					}
					num++;
				}
			}
	...
	public void Start(int vindex, int tindex)
	{
		...
		subHairXYZOffsets[0] = Vector3.zero;
		for (int i = 1; i < numHairs; i++)
		{
			if (globalSettings.bundleType == HairBundleType.Circular)
			{
				...
				subHairXYZOffsets[i] = new Vector3(x, y, z);
				if (debug)
				{
			...
			else
			{
				...
				subHairXYZOffsets[i] = new Vector3(num3, num4, z2);
			}
		}
