...
public class BakedSkinMesh : MonoBehaviour
{
	...
	private void DrawDebug(Matrix4x4 m, Material mat)
	{
		...
		{
			Graphics.DrawMesh(debugMesh, m, mat, 0, null, 0, null, false, false);
		}
	}
	...
	private void SelfBake1()
	{
		...
		for (int j = 0; j < numVertices; j++)
		{
			...
			bakedVertices[j] = vertexMatrix.MultiplyPoint3x4(originalVertices[j]);
			if (flag)
			{
				flag = false;
	...
	private void InitSkinnedMeshVars()
	{
		...
		for (int i = 0; i < numBones; i++)
		{
			...
			boneMatrices[i] = boneTransforms[i].localToWorldMatrix;
		}
		if (!alignTangents)
		{
			return;
