...
public class DAZPhysicsCharacter : MonoBehaviour
{
	...
	{
		LookAtWithLimits[] componentsInChildren = copyFrom.GetComponentsInChildren<LookAtWithLimits>();
		LookAtWithLimits[] array = componentsInChildren;
		foreach (Component component in array)
		{
			Transform value;
			if (TransformMap.TryGetValue(component.name, out value))
			{
				copyComponent(component, value.gameObject);
			}
		}
		SyncMaterialParameters[] componentsInChildren2 = copyFrom.GetComponentsInChildren<SyncMaterialParameters>();
		SyncMaterialParameters[] array2 = componentsInChildren2;
		foreach (Component component2 in array2)
		{
			Transform value2;
			if (TransformMap.TryGetValue(component2.name, out value2))
			{
				copyComponent(component2, value2.gameObject);
			}
		}
		AdjustJointTargetAndCOG[] componentsInChildren3 = copyFrom.GetComponentsInChildren<AdjustJointTargetAndCOG>();
		AdjustJointTargetAndCOG[] array3 = componentsInChildren3;
		foreach (Component component3 in array3)
		{
			Transform value3;
			if (TransformMap.TryGetValue(component3.name, out value3))
			{
				copyComponent(component3, value3.gameObject);
			}
		}
		BakedSkinMesh[] componentsInChildren4 = copyFrom.GetComponentsInChildren<BakedSkinMesh>();
		BakedSkinMesh[] array4 = componentsInChildren4;
		foreach (Component component4 in array4)
		{
			Transform value4;
			if (TransformMap.TryGetValue(component4.name, out value4))
			{
				copyComponent(component4, value4.gameObject);
			}
		}
		DAZHairMesh[] componentsInChildren5 = copyFrom.GetComponentsInChildren<DAZHairMesh>();
		DAZHairMesh[] array5 = componentsInChildren5;
		foreach (Component component5 in array5)
		{
			Transform value5;
			if (TransformMap.TryGetValue(component5.name, out value5))
			{
				copyComponent(component5, value5.gameObject);
			}
		}
		ForceReceiver[] componentsInChildren6 = copyFrom.GetComponentsInChildren<ForceReceiver>();
		ForceReceiver[] array6 = componentsInChildren6;
		foreach (Component component6 in array6)
		{
			Transform value6;
			if (TransformMap.TryGetValue(component6.name, out value6))
			{
				copyComponent(component6, value6.gameObject);
			}
		}
	}
	...
	public void createForceReceivers()
	{
		...
		{
			Transform value;
			if (TransformMap.TryGetValue(key, out value))
			{
			...
			if (!JointMap.TryGetValue(jointName, out value))
			{
				...
				if (transform != null)
				{
					...
					if ((bool)joint.connectedBody)
					{
						...
						Rigidbody value2;
						if (RBMap.TryGetValue(text, out value2))
						{
	...
	public void copyFollowPhysically()
	{
		...
		foreach (string fPJointName in copyFrom.getFPJointNames())
		{
			...
			FreeControllerV3 value2;
			if ((bool)targets && TargetsMap.TryGetValue(fPJointName, out value2))
			{
				value.follow = value2.transform;
				value2.followWhenOff = value.transform;
				Transform value3;
				if ((bool)followTree && FollowMap.TryGetValue(fPJointName, out value3))
				{
			...
			{
				Transform value4;
				if (FollowMap.TryGetValue(fPJointName, out value4))
				{
					value.follow = value4;
				}
			}
			...
			{
				FreeControllerV3 value5;
				if (TargetsMap.TryGetValue(freeControllerV.name, out value5))
				{
	...
	{
		Transform value;
		if (TransformMap.TryGetValue(trans, out value))
		{
			return value;
		}
		return null;
	...
	{
		Rigidbody value;
		if (RBMap.TryGetValue(joint, out value))
		{
			return value;
		}
		return null;
	...
	{
		SetCenterOfGravity value;
		if (CGMap.TryGetValue(joint, out value))
		{
			return value;
		}
		return null;
	...
	{
		FollowPhysicallySingle value;
		if (FPMap.TryGetValue(joint, out value))
		{
			return value;
		}
		return null;
	...
	public ConfigurableJoint getJoint(string joint)
	{
		...
		if (JointMap.TryGetValue(joint, out value))
		{
			return value;
		}
		return null;
