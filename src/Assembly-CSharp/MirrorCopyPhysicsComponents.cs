...
public class MirrorCopyPhysicsComponents : MonoBehaviour
{
	...
	public ConfigurableJoint getCopyFromJoint(string joint)
	{
		...
		ConfigurableJoint value;
		if (copyFromJointMap != null && copyFromJointMap.TryGetValue(joint, out value))
		{
			return value;
		}
		return null;
	...
	public Transform getCopyFromTransform(string trans)
	{
		...
		Transform value;
		if (copyFromTransformMap != null && copyFromTransformMap.TryGetValue(trans, out value))
		{
			return value;
		}
		return null;
	...
	public Rigidbody getCopyFromRB(string joint)
	{
		...
		Rigidbody value;
		if (copyFromRBMap != null && copyFromRBMap.TryGetValue(joint, out value))
		{
			return value;
		}
		return null;
	...
	private void copyCapsuleCollider(GameObject refgo, GameObject go)
	{
		...
		if (component != null)
		{
			...
			Vector3 center = MirrorVector(component.center);
			...
			if (!jointMap.TryGetValue(text, out value))
			{
				...
				if (transform != null)
				{
					...
					if ((bool)copyFromJoint.connectedBody)
					{
						...
						string text2 = Regex.Replace(input, replaceRegexp, withString);
						Rigidbody value2;
						if (RBMap.TryGetValue(text2, out value2))
						{
