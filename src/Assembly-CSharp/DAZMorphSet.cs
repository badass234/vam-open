...
public class DAZMorphSet : MonoBehaviour
{
	...
	public void InitSet()
	{
		...
		DAZMesh[] componentsInChildren = DAZMeshInitTransform.GetComponentsInChildren<DAZMesh>(true);
		...
		foreach (DAZMesh dAZMesh in array)
		{
			...
			{
				if (morph.morphValue != morph.startValue)
				{
					if (morph.visible)
					{
	...
	public void ApplySet()
	{
		...
		foreach (DAZMesh dAZMesh in array)
		{
			...
			foreach (DAZMorph morph2 in dAZMesh.morphBank.morphs)
			{
				if (morph2.visible)
				{
					morph2.morphValue = morph2.startValue;
				}
			}
		}
